 ...
 * 
 * 사원 수정(PROC_MOD_EMPLOYEE) 프로시져와 사원 삭제(PROC_DEL_EMPLOYEE) 프로시져를 생성하여 다음의
 * 기능이 동작하도록 한다.
 *    - EMPLOYEES 테이블의 사원정보를 수정/삭제 할 수 있는 최소한의 정보를 이용하여 프로시져가 동작하게 한다.
 *    - 사원의 정보를 수정할 때는 급여, 직무, 부서만 수정할 수 있게 한다.
 *    - 수정/삭제 된 사원의 부서의 맞추어 DEPARTMENTS 테이블의 TOTAL 컬럼의 총원을 증가 혹은 감소시키도록 한다.
 */

--전반적으로 구멍이 많은 풀이. 집에서 다시 해보고 부족한 부분 채워넣기
...
BEGIN
	...
	SELECT COUNT(*) INTO EXISTS_JOB FROM JOBS WHERE JOB_ID = IN_JOB_ID;  --입력한 JOB_ID 가 실제로 존재하는지 확인하기 위한 구문
	SELECT COUNT(*) INTO EXISTS_DEPT FROM DEPARTMENTS WHERE DEPARTMENT_ID = IN_DEPT_ID;  --입력한 DEPT_ID 가 실제로 존재하는지 확인하기 위한 구문
	IF EXISTS_JOB = 1 AND EXIST_DEPT = 1 THEN
		SELECT MIN_SALARY INTO EMP_SALARY FROM JOBS WHERE JOB_ID = IN_JOB_ID;
		INSERT INTO EMPLOYEES(EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, HIRE_DATE, JOB_ID, SALARY, DEPARTMENT_ID)
	   	      VALUES(EMP_ID, IN_FNAME, IN_LNAME, CONCAT(SUBSTR(UPPER(IN_FNAME), 1, 1), UPPER(IN_LNAME)), SYSDATE, IN_JOB_ID, EMP_SALARY, IN_DEPT_ID);
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE(ROW_COUNT || ' 개 행이 반영됨'); 
END;
...
BEGIN 
	...
	DBMS_OUTPUT.PUT_LINE(ROW_COUNT || ' 개 행 반영');
END;