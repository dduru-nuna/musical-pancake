...
/*
 * 사원테이블에서 SALARY 정보가 3,000 이하인 사원의 사번(EMPLOYEE_ID)
 ...
 */
SELECT EMPLOYEE_ID 사번, FIRST_NAME||' '||LAST_NAME 이름, JOB_ID 직무
  FROM EMPLOYEES
 ...
 */
SELECT DEPARTMENT_ID 부서ID, DEPARTMENT_NAME 부서명
  FROM DEPARTMENTS
 ...
 */
SELECT JOB_ID 직무ID, JOB_TITLE 직무명
  FROM JOBS
 ...
 */
SELECT SALARY, SALARY*(1+COMMISSION_PCT) "급여(커미션포함)"
  FROM EMPLOYEES
 ...
 */
SELECT DISTINCT DEPARTMENT_ID 부서ID, JOB_ID 직무ID
  FROM EMPLOYEES
