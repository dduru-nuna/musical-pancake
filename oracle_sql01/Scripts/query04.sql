...
SELECT EXTRACT(YEAR FROM SYSDATE) AS 년도
  ...
  FROM DUAL;
 --시간 관련해서는 SYSTIMESTAMP 에서 추출해야 한다
...
SELECT '1234' AS 문자타입
     ...
     , SYSDATE AS 날짜타입
  FROM DUAL;
--타입 확인할 때 results에서 ABC, 123, 시계표시 를 확인한다.
SELECT TO_CHAR(1234)
     , TO_CHAR(12.34)
     , TO_CHAR(SYSDATE)
  FROM DUAL;   

SELECT TO_CHAR(123456789, '999,999,999') AS 천단위구분포맷지정
     , TO_CHAR(123456, '000,000,000')
  FROM DUAL;
  
SELECT TO_CHAR(SYSDATE, 'YYYYMMDD') AS 형식지정1
     , TO_CHAR(SYSDATE, 'YYYY/MM/DD') AS 형식지정2
     , TO_CHAR(SYSDATE, 'YYYY-MM-DD') AS 형식지정3
     , TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') AS 형식지정4
  FROM DUAL;
  
SELECT TO_CHAR(SYSDATE, 'HH:MI:SS') AS 형식지정1
     , TO_CHAR(SYSDATE, 'HH24:MI:SS') AS 형식지정2
     , TO_CHAR(SYSDATE, 'HH24"시" MI"분" SS"초"') AS 형식지정3
  FROM DUAL;
...
SELECT TO_NUMBER('1234') AS A
  ...
  FROM DUAL;
--천단위 구분을 넣어서 숫자로 변환하려면 형식까지 써줘야한다
...
SELECT TO_DATE(20221111) AS A
  ...
  FROM DUAL;
--년 월 일 형식을 DATE로 변환하려면 형식까지 써줘야한다
...
SELECT 10 * 2 AS 계산결과1
  ...
  FROM DUAL;
--NULL 값이 하나라도 있으면 NULL이 반환. NVL 사용하면 NULL값을 다른 값으로 변환
 ...
 */ 
SELECT REGION_ID
     , DECODE(REGION_ID, 1, '유럽', 2, '아메리카', 3, '아시아', 4, '중앙아시아 및 아프리카', '호주') 변경
     , REGION_NAME
  FROM REGIONS;
--switch문의 defualt 같은 역할은 마지막에 써주면 된다. 어느값에도 해당하지 않으면 호주 반환
