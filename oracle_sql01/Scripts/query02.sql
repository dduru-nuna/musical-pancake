 ...
 *       조회 결과로 Record Set을 반환한다.
 *       Record Set에는 Record 가 포함되어 있으며, 이 Record는 0개가 나올 수 있다.
 */

/*
 * - SELECT 구문에는 SELECT 절, FROM 절, WHERE 절, ORDER BY 절 등으로 구성되어 있다.
 * - SELECT 절에는 Record Set 에 포함시킬 컬럼정보를 작성한다. * 은 모든 컬럼을 가르킴
...
SELECT EMPLOYEE_ID "사원 번호"
     ...
     , LAST_NAME AS 성
 FROM EMPLOYEES;
--공백으로 구분할 수 있고 AS 를 기입해서 구분할 수 있다. 
--별칭에 띄어쓰기가 들어갈 경우 ""를 이용할 수 있다.
...
SELECT EMPLOYEE_ID 사번
     , SALARY * 12 급여
  FROM EMPLOYEES;
--천단위 구분은 DBeaver에서 지원하는 기능. 환경설정 데이터포멧에서 설정가능
 ...
 *    NOT : 부정 (선택된 Record 가 아닌 Record를 Record Set에 반영)
 */
SELECT EMPLOYEE_ID
     , FIRST_NAME
     , LAST_NAME
 ...
 *   - 소괄호 안에 해당하는 값만 Record 만 Record Set에 포함된다.
 */
SELECT EMPLOYEE_ID
     , FIRST_NAME
     , LAST_NAME
...
--ESCAPE 문자로 \,?,a 등 사용가능. 언더바(_)이후 세글자 오는 ID 조회
