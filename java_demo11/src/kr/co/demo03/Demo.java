...
public class Demo {
	...
	public static void main(String[] args) {
		 ...
		 *    
		 * HashTable
		 *    - HashMap 과 동일하고 Thread Safe 기능을 지원한다.
		 *    
		 * Properties
		 *    - 키와 값을 String 타입으로 제한한 Map 컬렉션
		 *    - 주로 프로퍼티 파일을 읽어 그 안의 속성을 프로그램에서 사용할 수 있게 만들기 위해 쓰임
		 *    - 프로퍼티 파일은 주로 어플리케이션이 동작하는 과정에서 어플리케이션이 초기 동작에 
		 *      필요한 값이나 설정값을 읽어서 사용할 수 있게 만들어져 있다.
		 *      (어플리케이션이 처음 동작할 때 데이터베이스 연결과 관련된 프로퍼티 파일을 읽어서 연결 시도)
		 */
		
		// 제네릭 타입 키, 값 두 개 설정해야함
		...
		// 값 수정하려면 동일 키 찾아서 값 넣으면 된다. 키만 별도로 변경할 수는 없음
