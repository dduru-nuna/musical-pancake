...
/* 인덱스 재구성 : 테이블의 Record 를 삭제해도 색인 정보는 남아 있기 때문에 이를 정리하기 위해 사용 */
...
--실행계획(Ctrl + Shift + E) 보면 TABLE ACCESS (FULL) : FULL SCAN -> TABLE ACCESS (BY INDEX ~) 로 바뀜
--인덱스가 잘 적용된 것을 확인할 수 있다
SELECT * FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'IT';
--인덱스가 걸리는 컬럼은 LIKE 적용해서 쓸 수 없다. 인덱스 검색 안되서 속도 느려짐
DROP INDEX DEPT_NAME_IX;

SELECT * FROM USER_IND_COLUMNS WHERE TABLE_NAME = 'EMPLOYEES';

CREATE INDEX EMP_SALARY_IX ON EMPLOYEES(SALARY * 12);
SELECT * FROM EMPLOYEES WHERE (SALARY * 12) = 36960;
